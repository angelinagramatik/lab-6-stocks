...
class StocksManagement extends React.Component {
    ...
    fetchOwnedStock = (name) => {
        fetch('http://localhost:3003/stocks?name=' + name + '&user=' + this.state.user.id)
    ...
    fetchUser = () => {
        fetch('http://localhost:3003/users/' + this.state.user.id)
        .then(response => response.json())
        .then(user => {
            if (user[0]) {
    ...
    buyStock = () => {
        ...
        if (parseFloat(user.money) > parseFloat(this.state.stocks[this.state.selectedStockName].current_value)) {
            ...
            // document.getElementById('error').value = "You don't have enough money";
